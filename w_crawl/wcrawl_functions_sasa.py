...
class IterationProcessor(object):
    ...
    def process_iteration(self, n_iter, iter_group):
        ...
        data_dims = 3879  # total number of residues in the system 
        ...
        for iseg in range(nsegs):
            ...
            for _idx, traj_pattern in enumerate([self.parent_traj_pattern, self.child_traj_pattern]):
                ...
                try:
                    sasa = md.shrake_rupley(md.formats.NetCDFTrajectoryFile(traj_path).read_as_traj(self.child_topology), mode='residue')
                # If this traj doesn't exist, handle it gracefully and put NaNs in the coords
                except OSError:
                    sasa = numpy.full(shape=(data_dims), fill_value=numpy.nan)

                iteration_data_array[iseg, _idx] = sasa
