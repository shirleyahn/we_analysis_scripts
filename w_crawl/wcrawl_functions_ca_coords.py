...
class IterationProcessor(object):
    ...
    base_sim_path = '/mnt/NFS/original'

    # Store the location of the PDB file to be used as the topology
    topology_filename = f'{base_sim_path}/CONFIG/closed.pdb'

    # Define the pattern used for finding each segment's traj files
    child_traj_pattern = f'{base_sim_path}' + '/traj_segs/{n_iter:06d}/{seg_id:06d}/seg.rst'
    parent_traj_pattern = f'{base_sim_path}' + '/traj_segs/{n_iter:06d}/{seg_id:06d}/parent.rst'
    ...
    def process_iteration(self, n_iter, iter_group):
        ...
        result: (numpy.ndarray) In general this could be an object, which is
          ...
          (n_segments, n_timepoints, data_dims, 3), where dimension 0 indexes the segment,
          dimension 1 indexes the frame number, dimension 2 indexes which alpha carbon,
          and dimension 3 indexes the x/y/z coordinates of alpha carbons.
        '''
        # Find the number of segments in the iteration at hand
        print("starting",n_iter)
        nsegs = iter_group['seg_index'].shape[0]

        #TODO: revise code below according to the property of interest that you would like to w_crawl
        # The dimensionality of the data you wish to store
        data_dims = len(self.topology.select('name CA')) 
        
        # Create an array to hold your data
        iteration_data_array = numpy.zeros((nsegs, self.nframes, data_dims, 3))

        coords_indices = self.topology.select('name CA')
        ...
        for iseg in range(nsegs):
            for _idx, traj_pattern in enumerate([self.parent_traj_pattern, self.child_traj_pattern]):
                ...
                try:
                    if n_iter == 1 and traj_path[-10:] == 'parent.rst':
                        coords = numpy.squeeze(md.formats.AmberRestartFile(traj_path).read(coords_indices)[0])
                    else:
                        coords = numpy.squeeze(md.formats.AmberNetCDFRestartFile(traj_path).read(coords_indices)[0])
                # If this traj doesn't exist, handle it gracefully and put NaNs in the coords
                except OSError:
                    coords = numpy.full(shape=(data_dims, 3), fill_value=numpy.nan)
